...
func TestBasicArithmetic(t *testing.T) {
	...
	if !ret.Equal(&Expr{core.NUMBER, 3.0, nil, nil}) {
		t.Error("Expected 3.0")
	}

	ret = evalNew(parse("(- -100 200)"))

	if !ret.Equal(&Expr{core.NUMBER, -300.0, nil, nil}) {
		t.Error("Expected -300.0")
	}

	ret = evalNew(parse("(* 2 2)"))

	if !ret.Equal(&Expr{core.NUMBER, 4.0, nil, nil}) {
...
func TestNestedArithmetic(t *testing.T) {
	...
	if !ret.Equal(&Expr{core.NUMBER, 6.0, nil, nil}) {
